...
import {
  ...
  PricingInput,
  PricingResult,
} from "@/src/core/shared/utils/pricingEngine";
...
const EXTRAS_CATALOG: Omit<ExtraService, "seleccionado">[] = [
  {
    ...
    tipo: "EQUIPAJE_10KG",
    nombre: "Maleta 10 kg",
    descripcion: "Equipaje extra en bodega",
    precio: 35,
    icon: "🧳",
  },
  {
    ...
    tipo: "EQUIPAJE_23KG",
    nombre: "Maleta 23 kg",
    descripcion: "Maleta grande en bodega",
    precio: 55,
    icon: "🛄",
  },
  {
    id: "comida",
    tipo: "COMIDA",
    nombre: "Comida a bordo",
    descripcion: "Menú especial en vuelo",
    precio: 18,
    icon: "🍽️",
  },
  {
  ...
  },
  {
    ...
    tipo: "SALON_VIP",
    nombre: "Salón VIP",
    descripcion: "Acceso a sala de espera premium",
    precio: 45,
    icon: "🥂",
  },
  {
    ...
    tipo: "FAST_TRACK",
    nombre: "Fast Track",
    descripcion: "Embarque prioritario y fila",
    precio: 20,
    icon: "⚡",
  },
  {
    id: "pet",
    tipo: "MASCOTA",
    nombre: "Mascota en cabina",
    descripcion: "Transporte de mascota ≤8kg",
    precio: 80,
    icon: "🐾",
  },
];
...
export function useBookingManager() {
  ...
  const [nivelLealtad, setNivelLealtad] =
    useState<PricingInput["nivelLealtad"]>(undefined);
  useEffect(() => {
  ...
  const diasAnticipacion = useMemo(() => {
    ...
    const diff =
      (new Date(vuelo.fecha_vuelo).getTime() - Date.now()) / 86400000;
  ...
  const extrasSeleccionados = useMemo(
    () => extras.filter((e) => e.seleccionado),
    [extras],
  );
  const costExtras = useMemo(
    () => extrasSeleccionados.reduce((s, e) => s + e.precio, 0),
    [extrasSeleccionados],
  );

  // ── Total final ───────────────────────────────────────────────────────────
  const totalFinal = useMemo(
    () => (pricing?.total ?? 0) + costExtras,
    [pricing, costExtras],
  );
  ...
  const seleccionarVuelo = useCallback(
    (id: number, claseElegida: typeof clase) => {
      setVueloId(id);
  ...
  const seleccionarAsiento = useCallback(
    (seat: string) => {
      setAsiento(seat);
  ...
  const guardarPasajero = useCallback((data: PassengerData) => {
    ...
    if (data.telefono.replace(/\D/g, "").length < 8) {
      setError("Teléfono inválido (mínimo 8 dígitos).");
      return;
    }
    setError(null);
    setPasajero(data);
  ...
  const validarPromo = useCallback(
    async (code: string) => {
      setPromoCode(code);
      try {
        ...
        const match = promos.find(
          (p: any) =>
            p.CodigoPromocion?.toUpperCase() === code.toUpperCase() &&
            p.Activa === 1,
        );
        setPromoValid(!!match);
        setPromoDisc(
          match
            ? (pricing?.total ?? 0) * ((match.ValorDescuento ?? 15) / 100)
            : 0,
        );
        return !!match;
      } catch {
  ...
  const confirmarPago = useCallback(
    async (metodoPago: string, referencia: string) => {
      ...
      try {
        ...
        try {
          const reservasList = await backendApi.reservas.porPasajero(
            usuario?.id ?? 0,
          );
          const pendiente = reservasList
            .filter(
              (r: any) =>
                r.IdVuelo === vuelo.IdVuelo && r.EstadoReserva === "PENDIENTE",
            )
            .sort((a: any, b: any) => b.IdReserva - a.IdReserva)[0];
          if (pendiente) {
            idReserva = pendiente.IdReserva;
            codigoReserva = pendiente.CodigoReserva;
          }
        } catch {
          /* usa fallback */
        }
        ...
        if (puntosGanados > 0) {
          notificationStore.add(
            NotificationFactory.puntosGanados(puntosGanados),
          );
        }
