...
export default function Screen() {
  const [INCIDENTES, set_INCIDENTES] = useState<any[]>([]);
  const [PROHIBICIONES, set_PROHIBICIONES] = useState<any[]>([]);
  const [EMERGENCIAS_MEDICAS, set_EMERGENCIAS_MEDICAS] = useState<any[]>([]);
  useEffect(() => {
      backendApi.incidentesVuelo.listar().then(d => set_INCIDENTES(d)).catch(() => {});
      backendApi.vuelos.listar().then(d => set_PROHIBICIONES(d)).catch(() => {});
      backendApi.vuelos.listar().then(d => set_EMERGENCIAS_MEDICAS(d)).catch(() => {});
  }, []);
