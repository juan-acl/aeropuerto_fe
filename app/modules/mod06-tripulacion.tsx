...
export default function Tripulacion() {
  const [TRIPULANTES, set_TRIPULANTES] = useState<any[]>([]);
  const [CERTIFICACIONES_TRIPULACION, set_CERTIFICACIONES_TRIPULACION] = useState<any[]>([]);
  const [DISPONIBILIDADES, set_DISPONIBILIDADES] = useState<any[]>([]);
  useEffect(() => {
      backendApi.tripulacion.listar().then(d => set_TRIPULANTES(d)).catch(() => {});
      backendApi.tripulacion.listar().then(d => set_CERTIFICACIONES_TRIPULACION(d)).catch(() => {});
      backendApi.tripulacion.listar().then(d => set_DISPONIBILIDADES(d)).catch(() => {});
  }, []);

  // tripulacion comes from local mock (not yet in useBackend)
  const beTripulacion: any = null;
