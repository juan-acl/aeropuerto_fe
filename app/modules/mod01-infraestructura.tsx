...
export default function Infraestructura() {
  ...
  const [PISTAS, set_PISTAS] = useState<any[]>([]);
  const [PUERTAS, set_PUERTAS] = useState<any[]>([]);
  useEffect(() => {
      backendApi.aeropuertos.listar().then(d => set_AEROPUERTOS(d)).catch(() => {});
      backendApi.pistas.listar().then(d => set_PISTAS(d)).catch(() => {});
      backendApi.puertas.listar().then(d => set_PUERTAS(d)).catch(() => {});
  }, []);

  const { aeropuertos: aeData, isOnline } = useBackend();
  const beAeropuertos = aeData.source === 'backend' ? aeData.data : null;
