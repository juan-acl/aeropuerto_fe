...
export default function Mod08() {
  ...
  const [FACTURAS_DATA, set_FACTURAS_DATA] = useState<any[]>([]);
  const [PROMOCIONES_DATA, set_PROMOCIONES_DATA] = useState<any[]>([]);
  useEffect(() => {
      backendApi.reservas.porPasajero(0).then(d => set_RESERVAS(d)).catch(() => {});
      backendApi.vuelos.listar().then(d => set_FACTURAS_DATA(d)).catch(() => {});
      backendApi.promociones.listar().then(d => set_PROMOCIONES_DATA(d)).catch(() => {});
  }, []);
