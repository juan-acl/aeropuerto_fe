...
export default function Screen() {
  const [CONTRATOS_DATA, set_CONTRATOS_DATA] = useState<any[]>([]);
  const [NORMATIVAS, set_NORMATIVAS] = useState<any[]>([]);
  useEffect(() => {
      backendApi.vuelos.listar().then(d => set_CONTRATOS_DATA(d)).catch(() => {});
      backendApi.vuelos.listar().then(d => set_NORMATIVAS(d)).catch(() => {});
  }, []);
