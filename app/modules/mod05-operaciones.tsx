...
export default function Mod05() {
  ...
  useEffect(() => {
    backendApi.vuelos
      ...
      .then((d) => {
        ...
        setVuelos(data);
        set_CANCELACIONES(
          data.filter((v: any) => v.estado_vuelo === "CANCELADO"),
        );
        setCancelaciones(
          data.filter((v: any) => v.estado_vuelo === "CANCELADO"),
        );
    ...
    backendApi.retrasos
      ...
      .catch(() => {});
    backendApi.incidentesVuelo
      .listar()
      .then((d) => {
        set_INCIDENTES_VUELO(d || []);
        setIncidentes(d || []);
      })
      .catch(() => {});
    backendApi.combustible.pedidos
      .listar()
      .then((d) => {
        set_COMBUSTIBLES(d || []);
        setCombustibles(d || []);
  ...
  const handleSave = async () => {
    if (tab === "retrasos") {
      if (!form.id_vuelo || !form.minutos) {
        Alert.alert("Error", "ID Vuelo y minutos son requeridos");
        return;
      }
      setRetrasos((d) => [
        ...d,
        {
          id_retraso: Date.now(),
          id_vuelo: Number(form.id_vuelo),
          tipo_retraso: form.tipo || "OPERATIVO",
          causa: form.causa || "",
          minutos_retraso: Number(form.minutos),
          responsable: form.responsable || "",
          compensacion_pasajeros: 0,
        },
      ]);
    } else if (tab === "incidentes") {
      if (!form.id_vuelo || !form.desc) {
        Alert.alert("Error", "ID Vuelo y descripción requeridos");
        return;
      }
      setIncidentes((d) => [
        ...d,
        {
          id_incidente_vuelo: Date.now(),
          id_vuelo: Number(form.id_vuelo),
          tipo_incidente: form.tipo || "TECNICO",
          descripcion: form.desc,
          gravedad: form.gravedad || "BAJA",
          acciones_tomadas: form.acciones || "",
          reportado_por: form.reportado || "",
          fecha_incidente: new Date().toISOString(),
        },
      ]);
    } else if (tab === "cancelaciones") {
      if (!form.id_vuelo || !form.motivo) {
        Alert.alert("Error", "ID Vuelo y motivo requeridos");
        return;
      }
      setSaving(true);
      try {
        ...
        setCancelaciones((d) => [
          ...d,
          {
            id_cancelacion: Date.now(),
            id_vuelo: Number(form.id_vuelo),
            motivo_principal: form.motivo,
            motivo_detallado: form.detalle || "",
        ...
        setVuelos((d) =>
          d.map((v) =>
            v.id_vuelo === Number(form.id_vuelo)
        ...
        Alert.alert(
          "Error al cancelar",
          e.message ?? "No se pudo cancelar el vuelo.",
        );
        setSaving(false);
        return;
      } finally {
    ...
    try {
      if (vueloModal === "reprogramar") {
        if (!form.nueva_fecha || !form.nueva_salida || !form.nueva_llegada) {
          Alert.alert(
            "Requerido",
            "Fecha, hora de salida y llegada son obligatorios.",
          );
          setSaving(false);
          return;
        }
        ...
        Alert.alert(
          "Reprogramado",
          `El vuelo ${vueloSeleccionado.id_vuelo} fue reprogramado.`,
        );
      } else if (vueloModal === "puerta") {
        if (!form.id_puerta) {
          Alert.alert("Requerido", "ID de la puerta es obligatorio.");
          setSaving(false);
          return;
        }
        ...
        Alert.alert(
          "Puerta asignada",
          `Puerta ${form.id_puerta} asignada al vuelo ${vueloSeleccionado.id_vuelo}.`,
        );
        ...
        if (
          ...
          Alert.alert("Requerido", "Todos los campos son obligatorios.");
          setSaving(false);
          return;
        }
        ...
        Alert.alert(
          "Vuelo creado",
          "El vuelo fue programado exitosamente en estado PROGRAMADO.",
        );
      }
    } catch (e: any) {
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(
                "Embarque cerrado",
                `Vuelo #${vuelo.id_vuelo}: embarque cerrado.`,
              );
            } catch (e: any) {
              Alert.alert(
                "Error",
                e.message ?? "No se pudo cerrar el embarque.",
              );
            }
          },
        },
    ...
    Alert.alert("Cambiar Estado", `Estado actual: ${actual}`, [
      ...options.map((e) => ({
        ...
        onPress: () => {
          setVuelos((d) =>
            d.map((v) =>
              v.id_vuelo === id ? { ...v, estado_vuelo: e as any } : v,
            ),
          );
  ...
  const renderContent = () => {
    if (tab === "vuelos") {
      const data = vuelos.filter((v) =>
        `${v.numero_vuelo} ${v.aeropuerto_origen} ${v.aeropuerto_destino}`
          .toLowerCase()
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              icon=""
              text="Sin vuelos"
              sub="No hay vuelos que coincidan con la búsqueda"
            />
          }
          renderItem={({ item }) => (
            <View>
              ...
              <View style={s.vueloActions}>
                <TouchableOpacity
                  style={s.actionChip}
                  onPress={() =>
                    changeEstadoVuelo(item.id_vuelo, item.estado_vuelo)
                  }
                >
                ...
                <TouchableOpacity
                  style={s.actionChip}
                  onPress={() => {
                    setVueloSeleccionado(item);
                    setForm({});
                    setVueloModal("reprogramar");
                  }}
                >
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setVueloSeleccionado(item);
                    setForm({ tipo_vuelo: "NACIONAL" });
                    setVueloModal("puerta");
                  }}
                >
                  <Text style={s.actionChipT}>🚪 Puerta</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[s.actionChip, { borderColor: C.danger + "60" }]}
                  onPress={() => cerrarEmbarque(item)}
                >
                  <Text style={[s.actionChipT, { color: C.danger }]}>
                    🔒 Cerrar
                  </Text>
                </TouchableOpacity>
              </View>
    ...
    if (tab === "retrasos") {
      const data = retrasos.filter((r) =>
        `${r.id_vuelo} ${r.tipo_retraso} ${r.causa}`
          .toLowerCase()
          .includes(q.toLowerCase()),
      );
      return (
        <FlatList
          data={data}
          keyExtractor={(r) => String(r.id_retraso)}
          contentContainerStyle={{
            padding: 14,
            paddingBottom: 24,
            flexGrow: 1,
          ...
          ListEmptyComponent={
            <EmptyState icon="" text="Sin retrasos registrados" />
          }
          renderItem={({ item: r }) => (
            <DataCard
              ...
              onDelete={() =>
                setRetrasos((d) =>
                  d.filter((x) => x.id_retraso !== r.id_retraso),
                )
              }
            >
              ...
              <Text style={s.cardDetail}>
                {" "}
                Compensación:{" "}
                {r.compensacion_pasajeros ? " Aplicada" : "❌ Sin compensación"}
              </Text>
            </DataCard>
          )}
        />
    ...
    if (tab === "cancelaciones") {
      const data = cancelaciones.filter((c) =>
        `${c.id_vuelo} ${c.motivo_principal}`
          .toLowerCase()
          .includes(q.toLowerCase()),
      );
      return (
        <FlatList
          data={data}
          keyExtractor={(c) => String(c.id_cancelacion)}
          contentContainerStyle={{
            padding: 14,
            paddingBottom: 24,
            flexGrow: 1,
          ...
          renderItem={({ item: c }) => (
            <DataCard
              ...
              onDelete={() =>
                setCancelaciones((d) =>
                  d.filter((x) => x.id_vuelo !== c.id_vuelo),
                )
              }
            >
              <Text style={s.cardDetail}>
                {" "}
                {c.observaciones_operativas ?? "—"}
              </Text>
              <Text style={s.cardDetail}>
                {" "}
                {c.hora_salida_programada
                  ?.split("T")[1]
                  ?.substring(0, 5)} →{" "}
                {c.hora_llegada_programada?.split("T")[1]?.substring(0, 5)}
              </Text>
            </DataCard>
          )}
        />
    ...
    if (tab === "incidentes") {
      const data = incidentes.filter((i) =>
        `${i.id_vuelo} ${i.tipo_incidente} ${i.descripcion}`
          .toLowerCase()
          .includes(q.toLowerCase()),
      );
      return (
        <FlatList
          data={data}
          keyExtractor={(i) => String(i.id_incidente_vuelo)}
          contentContainerStyle={{
            padding: 14,
            paddingBottom: 24,
            flexGrow: 1,
          ...
          ListEmptyComponent={
            <EmptyState icon="" text="Sin incidentes registrados" />
          }
          renderItem={({ item: i }) => (
            <DataCard
              ...
              accentColor={
                i.gravedad === "ALTA" || i.gravedad === "CRITICA"
                  ? C.danger
                  : C.warning
              }
              onDelete={() =>
                setIncidentes((d) =>
                  d.filter(
                    (x) => x.id_incidente_vuelo !== i.id_incidente_vuelo,
                  ),
                )
              }
            >
    ...
    // Combustible
    const data = combustibles.filter((c) =>
      `${c.id_vuelo} ${c.tipo_combustible} ${c.proveedor}`
        .toLowerCase()
        .includes(q.toLowerCase()),
    );
    return (
      <FlatList
        data={data}
        keyExtractor={(c) => String(c.id_combustible)}
        contentContainerStyle={{ padding: 14, paddingBottom: 24, flexGrow: 1 }}
        ListEmptyComponent={
          <EmptyState icon="⛽" text="Sin registros de combustible" />
        }
        renderItem={({ item: c }) => (
          <DataCard
            ...
            onDelete={() =>
              setCombustibles((d) =>
                d.filter(
                  (x) => x.id_pedido_combustible !== c.id_pedido_combustible,
                ),
              )
            }
          >
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
      <ScreenHeader
        ...
        onAdd={() => {
          setForm({});
          tab === "vuelos" ? setVueloModal("crear") : setModal(true);
        }}
        addLabel={addLabel}
      />
      <TabBar
      ...
      {tab === "vuelos" && stats.demorados > 0 && (
        <View style={{ paddingHorizontal: 14, paddingTop: 10 }}>
          <AlertBanner
            type="warning"
            message={`${stats.demorados} vuelo${stats.demorados > 1 ? "s" : ""} con retraso. Retraso total acumulado: ${stats.minRetraso} min.`}
          />
        </View>
      )}
      ...
      >
        <FF
          ...
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, id_vuelo: t }))
          }
          keyboardType="numeric"
          required
          hint="Número de ID del vuelo afectado"
        />
        {tab === "retrasos" && (
          <>
            <FF
              ...
              value={form.tipo ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, tipo: t }))
              }
              hint="Ej: OPERATIVO, METEOROLOGICO, TECNICO"
            />
            <FF
              label="Minutos de Retraso"
              value={form.minutos ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, minutos: t }))
              }
              keyboardType="numeric"
              required
            />
        ...
        {tab === "incidentes" && (
          <>
            <FF
              ...
              hint="Ej: TECNICO, SEGURIDAD, MEDICO"
            />
            <FF
              label="Gravedad"
              value={form.gravedad ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, gravedad: t }))
              }
              hint="BAJA / MEDIA / ALTA / CRITICA"
              required
            />
            <FF
              label="Descripción"
            ...
            <FF
              ...
              value={form.acciones ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, acciones: t }))
              }
              multiline
            />
            <FF
              label="Reportado Por"
              value={form.reportado ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, reportado: t }))
              }
            />
          </>
        )}
        {tab === "cancelaciones" && (
          <>
            <FF
              ...
              value={form.motivo ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, motivo: t }))
              }
              required
              hint="Ej: METEOROLOGICO, TECNICO, HUELGA"
            />
            <FF
              label="Detalle"
              value={form.detalle ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, detalle: t }))
              }
              multiline
            />
            <FF
              label="Pasajeros Reubicados"
              value={form.reubicados ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, reubicados: t }))
              }
              keyboardType="numeric"
            />
            <FF
              label="Costo Compensación (Q)"
              value={form.costo ?? ""}
              onChangeText={(t: string) =>
                setForm((f: any) => ({ ...f, costo: t }))
              }
      ...
      {/* Modal: Crear Vuelo */}
      <FormModal
        visible={vueloModal === "crear"}
        title="Crear Vuelo"
        onClose={() => {
          setVueloModal(null);
          setForm({});
        }}
        onSave={handleVueloAction}
        saveLabel={saving ? "Creando..." : "Crear Vuelo"}
      >
        <FF
          label="ID Programa de Vuelo"
          required
          value={String(form.id_programa ?? "")}
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, id_programa: t }))
          }
          keyboardType="numeric"
          hint="ID del programa (ruta programada)"
        />
        <FF
        ...
        <FF
          ...
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, hora_llegada: t }))
          }
          placeholder="HH:MM:SS"
        />
        <FF
          label="ID Modelo de avión"
          required
          value={String(form.id_modelo ?? "")}
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, id_modelo: t }))
          }
          keyboardType="numeric"
        />
        <FF
          ...
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, matricula: t }))
          }
          autoCapitalize="characters"
          placeholder="Ej: TG-ANA"
        />
      </FormModal>

      {/* Modal: Reprogramar Vuelo */}
      <FormModal
        visible={vueloModal === "reprogramar"}
        title={`Reprogramar Vuelo #${vueloSeleccionado?.id_vuelo ?? ""}`}
        onClose={() => {
          setVueloModal(null);
          setForm({});
        }}
        onSave={handleVueloAction}
        saveLabel={saving ? "Reprogramando..." : "Reprogramar"}
      >
        <FF
          label="Nueva fecha"
          required
          ...
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, nueva_fecha: t }))
          }
          placeholder="YYYY-MM-DD"
        />
        <FF
          label="Nueva hora de salida"
          required
          value={form.nueva_salida ?? ""}
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, nueva_salida: t }))
          }
          placeholder="HH:MM:SS"
        />
        <FF
          label="Nueva hora de llegada"
          required
          value={form.nueva_llegada ?? ""}
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, nueva_llegada: t }))
          }
          placeholder="HH:MM:SS"
        />
      </FormModal>
      ...
      <FormModal
        visible={vueloModal === "puerta"}
        title={`Asignar Puerta — Vuelo #${vueloSeleccionado?.id_vuelo ?? ""}`}
        onClose={() => {
          setVueloModal(null);
          setForm({});
        }}
        onSave={handleVueloAction}
        saveLabel={saving ? "Asignando..." : "Asignar"}
      >
        <FF
          label="ID Puerta de embarque"
          required
          value={String(form.id_puerta ?? "")}
          onChangeText={(t: string) =>
            setForm((f: any) => ({ ...f, id_puerta: t }))
          }
          keyboardType="numeric"
          hint="ID numérico de la puerta en el sistema"
        />
        <FSelect
          label="Tipo de vuelo"
          value={form.tipo_vuelo ?? "NACIONAL"}
          onChange={(v: string) =>
            setForm((f: any) => ({ ...f, tipo_vuelo: v }))
          }
          options={[
